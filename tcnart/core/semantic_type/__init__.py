...
def _content_from_type_and_id(ct: ContentTypes, semantic_bits: int) -> ContentTypeConfigStorage:
    masked = semantic_bits & constants.SEMANTIC_TYPE_MASK
...
class SemanticType:
    ...
    def to_identifier(self) -> IdentifierStorageType:
        content_bits = _content_storage_to_u64(self.content_type) & constants.SEMANTIC_TYPE_MASK
        base_bits = self.base_type.base_type_identifier() & constants.BASE_TYPE_MASK
        ...
        return int((content_bits | base_bits) & 0xFFFFFFFFFFFFFFFF)
    ...
    def set_content_type(self, ct: ContentTypes, config: Optional[int]) -> None:
        semantic_bits = (config or 0) & constants.SEMANTIC_TYPE_MASK
    ...
    def __repr__(self) -> str:
        ...
        base_part = value & constants.BASE_TYPE_MASK

        st = (base_part & constants.SCALAR_TYPE_MASK) >> constants.SCALAR_TYPE_OFFSET
        cat = (base_part & constants.CARDINALITY_TYPE_MASK) >> constants.CARDINALITY_TYPE_OFFSET
        ct = (base_part & constants.CONTAINER_TYPE_MASK) >> constants.CONTAINER_TYPE_OFFSET
        mrt = (base_part & constants.MEMORY_REPRESENTATION_TYPE_MASK) >> constants.MEMORY_REPRESENTATION_TYPE_OFFSET
